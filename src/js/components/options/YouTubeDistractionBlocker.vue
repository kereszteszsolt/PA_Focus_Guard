  ...
  <div class="distractions-container">
    <div class="distractions-list">
      <table>
        ...
        <tbody>
        <tr v-for="(distraction, index) in youtubeDistractionsList" :key="index">
...
export default {
  ...
  methods: {
      ...
      chrome.storage.sync.get(this.storageName, (result) => {
        if (result[this.storageName]) {
          JSON.parse(result[this.storageName]).forEach((storedDistraction, index) => {
            this.youtubeDistractionsList.forEach((youTubeDistractionItem, index) => {
              if (youTubeDistractionItem.name === storedDistraction.name) {
                this.youtubeDistractionsList[index].activeRule = storedDistraction.activeRule;
                this.youtubeDistractionsList[index].permanently = storedDistraction.permanently;
              }
            });
          });
        } else {
    ...
    saveListOfDistractions() {
      chrome.storage.sync.set({[this.storageName]: JSON.stringify(this.youtubeDistractionsList)});
    }
  }
};
</script>

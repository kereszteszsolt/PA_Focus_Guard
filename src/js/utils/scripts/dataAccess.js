    ...
    return new Promise((resolve, reject) => {
        chrome.storage.sync.get([storageName], function (result) {
            if (chrome.runtime.lastError) {
                reject(new Error(`Error loading data for '${storageName}': ${chrome.runtime.lastError.message}`));
            } else {
...
export function saveData(storageName, data) {
    return new Promise((resolve, reject) => {
        ...
        chrome.storage.sync.set({[storageName]: serializedData}, function () {
            if (chrome.runtime.lastError) {
                reject(new Error(`Error saving data for '${storageName}': ${chrome.runtime.lastError.message}`));
            } else {
                resolve();
            }
        });
